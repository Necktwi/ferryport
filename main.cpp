...
std::map<string, camState> camStringState;
...
static void camStringStateInit() {
	...
	camStringState["CAM_STREAM"] = CAM_STREAM;
	camStringState["CAM_STREAM_N_RECORD"] = CAM_STREAM_N_RECORD;
}
...
public:
	...
	static pid_t setCamState(string cam) {
		...
		if (!cs.disable) {
			...
			cs.pid = fcpid;
			cs.state = ns;
			model["system"][machineName]["cameras"][cam]["state"] = camStateString[ns];
	...
	static string getCamsWithStateString() {
		...
		while (csit != csl.end()) {
			...
			//model["system"][machineName]["cameras"][csit->second.cam]["state"] = camStateString[csit->second.state];
			csit++;
		}
		return strCameras;
	}
...
void setState() {
	...
	i = 0;
	int k = 0;
	FFJSON& cams = model["system"][machineName]["cameras"];
	while (vd[i].length() != 0) {
		k = 0;
		if (!csList::isCamDisabled(vd[i]) &&
				(csList::getCamNewState(vd[i]) != CAM_PREVIOUS_STATE ||
				strcmp((const char*) cams[vd[i]]["state"],
				(const char*) cams[vd[i]]["newState"]) != 0)) {
			fold = path + vd[i];
			...
			if (serverType == REST) {
				csList::setNewCamState(vd[i],
						camStringState[string((const char*) cams[vd[i]]["newState"])]);
			}
...
camState systemStateChange() {
	...
	model["system"][machineName]["location"] = strGPS;
	fflush(stdout);
	...
	model["system"][machineName]["ip"] = currentIP;
	model["system"][machineName]["signalstrength"] = -1;
