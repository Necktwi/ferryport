...
static int
callback_echo(struct libwebsocket_context *context, struct libwebsocket *wsi, enum libwebsocket_callback_reasons reason, void *user, void *in, size_t len) {
...
#endif
    if (use_ssl && !client) {
        info.ssl_cert_filepath = "certs/ferryport.ferryfair.cert";
        info.ssl_private_key_filepath = "certs/ferryport.ferryfair.key";
    }
    info.gid = -1;
    info.uid = -1;
    info.options = opts;
